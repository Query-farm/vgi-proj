...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-proj
...
# ATTACH (`${VGI_PROJ_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run proj_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/proj_worker.py`.
# Optional:
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_PROJ_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/proj-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/proj-unix-server.log"
    ...
    export VGI_PROJ_WORKER="unix://$SOCK"
    ;;
