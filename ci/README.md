# CI: the vgi-proj worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-proj
...
1. **Install the worker** — `uv sync --frozen` into a venv. `proj_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run proj_worker.py`; its inline deps (`pyproj`, which bundles PROJ and
   its data grids in the wheel) resolve from PyPI with no native install or
   network grid download.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_PROJ_WORKER` at `uv run proj_worker.py`, warms the
...
VGI_PROJ_WORKER="uv run --python 3.13 proj_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 proj_worker.py`.
